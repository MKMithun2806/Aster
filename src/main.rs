...
mod windows_app {
    ...
    use windows::{
        ...
        Win32::{
            ...
            Graphics::Gdi::{
                ...
                GetMonitorInfoW, GetStockObject, InvalidateRect, LineTo, MonitorFromWindow,
                MoveToEx, RoundRect, ScreenToClient, SelectObject, SetBkMode, SetTextColor,
                SetWindowRgn, AC_SRC_ALPHA, AC_SRC_OVER, BITMAPINFO, BITMAPINFOHEADER, BI_RGB,
            ...
            UI::{
                ...
                Input::KeyboardAndMouse::{
                    ...
                    VK_F11, VK_F5, VK_MENU, VK_RETURN,
                },
                WindowsAndMessaging::{
                    ...
                    WM_LBUTTONUP, WM_MOUSEMOVE, WM_MOUSEWHEEL, WM_NCCREATE, WM_PAINT,
    ...
    impl App {
        fn new(hwnd: HWND, environment: ICoreWebView2Environment) -> AppResult<Self> {
            ...
            unsafe {
                let _ =
                    WindowsAndMessaging::SetTimer(Some(app.hwnd), HOVER_DETECT_TIMER_ID, 100, None);
            }
            Ok(app)
        }
        ...
        fn add_folder_rows_recursive(&self, folder_id: usize, rows: &mut Vec<SidebarRow>) {
            let child_folders: Vec<&Folder> = self
                ...
                .iter()
                .filter(|f| {
                    f.workspace_id == self.active_workspace && f.parent_id == Some(folder_id)
                })
                .collect();
        ...
        fn switch_to(&mut self, index: usize, wake_up: bool) {
            ...
            for (i, tab) in self.tabs.iter().enumerate() {
                unsafe {
                    let _ = tab.controller.SetIsVisible(
                        i == index && tab.workspace_id == workspace_id && !tab.unloaded,
                    );
                }
            }
            self.active = index;
            ...
            if let Some(rect) = row_rect {
                unsafe {
                    ...
                    if let Ok(edit) = edit_hwnd {
                        ...
                        OLD_RENAME_EDIT_PROC =
                            mem::transmute(WindowsAndMessaging::SetWindowLongPtrW(
                                edit,
                                GWLP_WNDPROC,
        ...
        fn command_suggestions(&self) -> Vec<(Option<usize>, String, String)> {
            ...
            // 1. Get visited history sites matching query, sorted by frecency score descending
            ...
            // 2. Add top matching history suggestions to rows first (guarantees that the top frecent site is the first recommendation)
            ...
            // 3. Add open tabs that match the query
            if rows.len() < 5 {
        ...
        fn layout(&self) {
            ...
            for (i, tab) in self.tabs.iter().enumerate() {
                unsafe {
                    ...
                    let _ = tab
                        .controller
                        .SetIsVisible(Some(i) == self.active_tab_index() && !tab.unloaded);
                }
            }
            if clip_changed {
                    ...
                    for (row, row_rect) in self.sidebar_row_rects() {
                        match row {
                            SidebarRow::Label(label) => {
                                self.paint_sidebar_label(hdc, label, row_rect)
                            }
            ...
            unsafe {
                ...
                let row_hover = self.hover_target == Some(HoverTarget::ModeRow)
                    || matches!(
                        self.hover_target,
                        Some(
                            HoverTarget::ModeAuto | HoverTarget::ModeDark | HoverTarget::ModeLight
                        )
                    );
        ...
        fn paint_command_popup(&self, hdc: HDC) {
            unsafe {
                ...
                for (row_index, (tab_index, title, url)) in
                    ...
                    match hit {
                        SidebarHit::WorkspaceButton(id) => {
                            ...
                            draw_outline(
                                hdc,
                                rect,
                                if active { COLOR_ACCENT } else { 0x2f2f2f },
                                14,
                            );
                            let label = self
                                .workspaces
            ...
            unsafe {
                match self.drop_target {
                    ...
                    Some(DropTarget::Folder(folder_id)) => {
                        if let Some((_, rect)) = self.sidebar_row_rects().into_iter().find(
                            |(row, _)| matches!(row, SidebarRow::Folder(id) if *id == folder_id),
                        ) {
                            let line_rect = RECT {
                                left: rect.left + 4,
                                top: rect.bottom - 2,
            ...
            match hit {
                ...
                SidebarHit::Tab(index) => {
                    ...
                    self.open_overlay_menu(
                        x,
                        y,
                        MenuTarget::Sidebar(SidebarHit::Tab(index)),
                        items,
                    );
                }
            }
        ...
        fn run_menu_command(&mut self, target: MenuTarget, id: usize) {
            match target {
                ...
                MenuTarget::ForwardHistory(index) => {
                    self.navigate_to_history_item(index, id, false)
                }
            }
            self.refresh();
        }
        ...
        fn start_drag_candidate(&mut self, x: i32, y: i32) {
            ...
            if let Some(SidebarHit::Tab(index)) = self.hit_sidebar(x, y) {
                if let Some((_, row)) = self.sidebar_row_rects().into_iter().find(
                    |(row, _)| matches!(row, SidebarRow::Tab(row_index) if *row_index == index),
                ) {
                    let audio_hit = self
                        .tabs
                        .get(index)
            ...
            unsafe {
                if !self.fullscreen {
                    ...
                    if GetMonitorInfoW(monitor, &mut monitor_info).as_bool() {
                        let _ =
                            SetWindowLong(self.hwnd, GWL_STYLE, (WS_POPUP | WS_VISIBLE).0 as isize);
    ...
    extern "system" fn window_proc(
        ...
        match msg {
            ...
            WM_SETCURSOR => unsafe {
                WindowsAndMessaging::DefWindowProcW(hwnd, msg, w_param, l_param)
            },
            WM_ERASEBKGND => LRESULT(1),
            WM_CLOSE => {
                unsafe {
    ...
    unsafe fn draw_bitmap_fit(hdc: HDC, rect: RECT, bitmap: &FaviconBitmap, dimmed: bool) {
        ...
        let scale =
            (target_w as f32 / bitmap.width as f32).min(target_h as f32 / bitmap.height as f32);
    ...
    mod tests {
        ...
        fn test_strip_google_transient_params() {
            ...
            assert_eq!(
                strip_google_transient_params(
                    "https://www.google.com/search?q=cat&zx=123&no_sw_cr=1"
                ),
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    windows_app::main().map_err(|error| Box::new(error) as Box<dyn std::error::Error>)
}
